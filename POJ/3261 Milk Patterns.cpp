...
void sortFirstChar(int n)
{
    ...
    for(int i=0 ; i<n ; i++)
    {
        bh[i] = (i==0  || quality[SA[i]]!=quality[SA[i-1]]);
        b2h[i] = false;
    }
    return;
}
...
void buildLCP(int n)
{
    ...
    for(int i =0 ; i< n ; i++)
    {
        ...
        int j = SA[k-1];
        while(quality[i+len]==quality[j+len]) len++;
        lcp[k] = len;
        if(len) len--;
    }
    return;
...
int findSol(int k)
{
    ...
    for (int i = 1; i<k-1; i++)
        pq.push(data(lcp[i],i));

    for(int i=k-1, j=1; i<SIZE(quality); i++,j++)
    {
        pq.push(data(lcp[i],i));
        while(pq.top().pos<j) pq.pop();
        mn = max(mn, pq.top().n);
    }
    return mn;
}
int main()
{
    ...
    buildSA(SIZE(quality));
    buildLCP(SIZE(quality));
    printf("%d",findSol(k));
    quality.clear();
    return 0;
}
