...
public abstract class EditorLanguage
{
	...
	/**
	 * The prfix to variables that will appear in test code as test parameters
	 ...
	 * @see     #sEndTimeVarName
	 */
	private static final String sStartTimeVarName = "startTime";
	/**
	 * Name of the variable in test case executor that will hold the time of
	 ...
	 */
	private static final String sEndTimeVarName = "endTime";
	/**
	 ...
	 */
	private static final String sTestEndExpr = sMainResVarName + " = " + sRunTestFuncName;
	//============================================================================
	 ...
	 * test code generation.
	 */
	private EditorDataType retType;
	/**
	 ...
	 */
	private void makeCStyleParams(StringBuilder res, EditorDataType[] paramTypes,
	...
	public final Skeleton getSkeleton(ClassDecl cl)
	{
		...
		// Finally place comment about plugin version
		output.append(sLineComment).append("Powered by ")
				.append(KawigiEdit.versionString).append('!');
	 ...
	 *                      return <code>this</code> to make possible convinient call chains
	 */
	private EditorLanguage methodCall(CharSequence objName, CharSequence methodName)
	{
	 ...
	 *                  return <code>this</code> to make possible convinient call chains
	 */
	private EditorLanguage arrayLensUnequal(String varLeft, String varRight)
	{
	                                ...
	                                EditorDataType type, String resVarName)
	{
		if (type.isArrayType()) {
			...
			iff().arrayLensUnequal(varLeft, varRight).then();
				text(resVarName).text(" = false").endCodeLine();
			els();
	 ...
	 *          return <code>this</code> to make possible convinient call chains
	 */
	private EditorLanguage passTestParams()
	{
	 ...
	 * time.
	 */
	private void callProblemSolver()
	{
	...
	private void printTestOutputInfo()
	{
		...
		elseIf().timeDiff(sStartTimeVarName, sEndTimeVarName).text(" >= 2").then();
			...
			text(sCallerResVarName).text(" = false").endCodeLine();
	...
	private void makeTestCaller()
	{
		...
		funcDefPostfix(EditorDataType.Boolean);
			...
			text(sCallerResVarName).text(" = true").endCodeLine();
			printTestOutputInfo();
	...
	private void callTest(int testNum, Test t)
	{
		...
		text(sTestEndExpr).text('(').text(testNum).comma()
		...
		// Test is over. For beauty purposes insert one empty line.
		markTestEnd();
		endLine();
	}
	...
	private void mainSub()
	{
		mainSubDef();
			...
			text(sMainResVarName).text(" = true").endCodeLine();
			endLine();
			...
			iff().text(sMainResVarName).then();
				println().text("\"You're a stud (at least on the example cases)!\"").endPrintln();
			els();
	...
	private void addTestCase()
	{
		...
		addTestCase(cl, params, parsedParams[paramTypes.length]);
	}
	                        ...
	                        StringBuilder aResult)
	{
		...
		classProblem.addTest(aParams, aResult);
	}
	                                   ...
	                                   StringBuilder testCode)
	{
		...
		if (0 <= startInd) {
			...
			if (0 <= endInd) {
				...
				matTestEnd = Pattern.compile(sTestEndExpr, Pattern.DOTALL)
