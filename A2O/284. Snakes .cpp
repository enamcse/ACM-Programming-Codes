...
int main()
{
    ...
    while(t--)
    {
        ...
        n = x*(LL)y;
        if(!(x&1) && !(y&1)) n--;
        matrix ans=bigmod(base,n-1, 3); //n-number of baseconditions+1

        v1 = (int)((a*(LL)v+b)%mod);
        sol = (int)((((ans.x[3][1]*v1)%mod) + ((ans.x[3][2])%mod)+ ((ans.x[3][3] *v)%mod) )%mod);
        printf("%d\n",sol);
    }
    return 0;
}
