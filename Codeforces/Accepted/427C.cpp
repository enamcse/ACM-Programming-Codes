...
int col[SZ],low[SZ],tim[SZ],timer, cost[SZ];
...
int main()
{
...
#endif // ENAM
    int i,j,t,cas=0,u,v;
    ll ans=1, weight, cnt , now;

    scanf("%d",&n);

    fr(i,0,n-1) scanf("%d",&cost[i]);
    ...
    TarjanSCC();
    fr(i,0,components-1) adj[i].clear();
    fr(i,0,n-1) adj [ group_id[i] ].pb(cost[i]);
